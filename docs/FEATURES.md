...
올인원 Docker 컨테이너로 통합된 HanaNaviLite의 핵심 기능들입니다.
...
*   **올인원 웹 인터페이스**: FastAPI에서 빌드된 React SPA를 직접 서빙하여 단일 URL로 모든 기능 제공
*   **GPU 가속 실시간 스트리밍**: RTX 5080 GPU를 활용한 고속 임베딩 처리 및 LLM 스트리밍 답변
*   **Evidence Panel**: 답변의 근거가 된 소스 문서를 명확하게 제시하여 사용자가 직접 내용을 확인할 수 있습니다
*   **단일 컨테이너 구조**: 복잡한 마이크로서비스 없이 하나의 컨테이너로 모든 기능 통합
*   **자동 GPU 감지**: CUDA 사용 가능 여부를 자동으로 감지하여 CPU/GPU 모드 선택
...
*   **경량화 및 GPU 최적화**: 32GB RAM 환경에서 안정적 운영, RTX 5080 GPU를 활용한 고속 임베딩 처리
*   **올인원 도커 컨테이너**: 복잡한 설치 없이 `docker-compose up -d` 명령 하나로 전체 시스템 실행
*   **포트 통일**: 8011 포트 하나로 API 서버와 웹 UI 동시 제공